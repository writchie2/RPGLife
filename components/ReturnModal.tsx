...
const ReturnModal: React.FC<ReturnModalProps> = ({ visible, onClose }) => {
        ...
        <View style={styles.modalContainer}>
          <ScrollView style={styles.scrollContainer}>
            ...
            {overdueQuests.length > 0 && (
              <View style={styles.contentContainer}>
                <Text style={styles.contentText}>
                  You have {overdueQuests.length}{" "}
                  {overdueQuests.length == 1 ? "quest" : "quests"} overdue!
                </Text>
                <Text style={styles.contentText}>
                  You lost {calcQuestDaysOverdue(overdueQuests) * 100} exp since
                  your last login. Start completing those quests!
                </Text>
                ...
                <View style={styles.dropdownContainer}>
                  ...
                  <QuestsList quests={userData?.quests || []} mode="overdue" />
                </View>
              </View>
            )}
            {overdueQuests.length <= 0 && (
              <View style={styles.contentContainer}>
                <Text style={styles.contentText}>
                  Great job, you have no quests overdue!
                </Text>
              </View>
            )}
            ...
            {dueSoonQuests.length > 0 && (
              <View style={styles.contentContainer}>
                <View style={styles.dropdownContainer}>
                  ...
                  <QuestsList quests={dueSoonQuests} mode="active" />
                </View>
              </View>
            )}
            ...
            {deterioratedSkills.length > 0 && (
              <View style={styles.contentContainer}>
                <Text style={styles.contentText}>
                  You have {deterioratedSkills.length}{" "}
                  {deterioratedSkills.length == 1 ? "skill" : "skills"}{" "}
                  deteriorating!
                </Text>
                <Text style={styles.contentText}>
                  Skills lose 500 exp every week they are archived! Re-Activate
                  them and start some quests!
                </Text>
                <View style={styles.dropdownContainer}>
                  <View style={styles.section}>
                    <View style={styles.sectionTitleContainer}>
